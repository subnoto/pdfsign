...
func TestSignPDFInitials(t *testing.T) {
	...
	for _, tc := range tests {
		...
		_, err = SignFile(lastSignedFile, tmpfile.Name(), SignData{
			Signature: SignDataSignature{
				Info: SignDataSignatureInfo{
					Name: tc.signerName,
					Date: time.Now().Local(),
...
// TestSignPDFApplePreview tests signing a PDF created with Apple Preview
// This ensures compatibility with PDFs generated by macOS Preview application
func TestSignPDFApplePreview(t *testing.T) {
	cert, pkey := loadCertificateAndKey(t)
	...
	// Verify the PDF can be read before signing
	rdr, err := pdf.NewReader(inputFile, finfo.Size())
	if err != nil {
		t.Fatalf("Failed to read input PDF: %v", err)
	}
	t.Logf("Apple Preview PDF has %d pages", rdr.NumPage())
	...
	// Sign the PDF with certification signature
	_, err = SignFile(inputFilePath, tmpfile.Name(), SignData{
		Signature: SignDataSignature{
			Info: SignDataSignatureInfo{
				Name:        "Test Signer",
				Location:    "Test Location",
				Reason:      "Testing Apple Preview PDF compatibility",
				ContactInfo: "test@example.com",
				Date:        time.Now().Local(),
			},
			CertType:   CertificationSignature,
	...
	if err != nil {
		t.Fatalf("Failed to sign Apple Preview PDF: %v", err)
	}

	// Verify the signed file
	verifySignedFile(t, tmpfile, originalFileName)

	// Additional verification: ensure the signed PDF can still be read
	signedFile, err := os.Open(tmpfile.Name())
	if err != nil {
		t.Fatalf("Failed to open signed file: %v", err)
	}
	...
	if signedRdr.NumPage() != rdr.NumPage() {
		t.Errorf("Page count mismatch: original=%d, signed=%d", rdr.NumPage(), signedRdr.NumPage())
	}

	t.Logf("✅ Successfully signed and verified Apple Preview PDF (testfile60.pdf)")
}
